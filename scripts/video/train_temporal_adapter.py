"""Train a temporal video adapter from generic frozen feature files."""
...
def evaluate(model: TemporalVideoAdapter, features: dict) -> dict:
    model.eval()
    with torch.no_grad():
...
def main() -> None:
    ...
    model = TemporalVideoAdapter(train["frames"].shape[-1], args.hidden_dim, train["frames"].shape[1]).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"epoch={epoch:03d} train_loss={sum(losses) / len(losses):.5f} dev={dev_report}", flush=True)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "state": best["state"], "dim": model.dim, "hidden_dim": model.hidden_dim, "max_frames": model.max_frames,
            "seed": args.seed, "best_epoch": best["epoch"], "dev_report": best["dev_report"],
            "train_features": str(args.train_features), "dev_features": str(args.dev_features),
        },
        args.output,
    )
    print(f"checkpoint={args.output} best_epoch={best['epoch']} dev={best['dev_report']}")
